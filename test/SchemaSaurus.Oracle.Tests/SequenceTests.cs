    ...
    : SchemaReaderTestBase(databaseFixture)
{
    ...
    public async Task WhenReadingSequencesThenOrderNumberSequenceExists()
    {
        ...
        model.Sequences.Should().Contain(s => s.SchemaQualifiedName.Name == OrderNumberSequenceName);
    }
    ...
    {
        var model = await GetDatabaseModelAsync();
        var sequence = model.Sequences.First(s => s.SchemaQualifiedName.Name == OrderNumberSequenceName);

        sequence.SchemaQualifiedName.Schema.Should().Be(model.DefaultSchemaName);
    }
