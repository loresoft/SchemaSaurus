    ...
    : SchemaReaderTestBase(databaseFixture)
{
    ...
    public async Task WhenReadingUserDefinedTypesThenIdentifierTableExists()
    {
        ...
        model.UserDefinedTypes.Should().Contain(u => u.SchemaQualifiedName.Name == "IdentifierTable");
    }
    ...
    {
        var model = await GetDatabaseModelAsync();
        var udt = model.UserDefinedTypes.First(u => u.SchemaQualifiedName.Name == "IdentifierTable");

        udt.SchemaQualifiedName.Schema.Should().Be("dbo");
    }
