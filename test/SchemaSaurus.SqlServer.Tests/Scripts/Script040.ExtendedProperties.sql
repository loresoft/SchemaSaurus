IF NOT EXISTS (
    ...
    WHERE [class] = 0
      AND [major_id] = 0
      AND [minor_id] = 0
      AND [name] = N'SchemaSaurus:Environment'
)
BEGIN
    EXEC sys.sp_addextendedproperty
        @name = N'SchemaSaurus:Environment',
...
IF NOT EXISTS (
    ...
    WHERE ep.[class] = 1
      AND ep.[major_id] = OBJECT_ID(N'[dbo].[User]')
      AND ep.[minor_id] = 0
      AND ep.[name] = N'SchemaSaurus:AggregateRoot'
)
BEGIN
    EXEC sys.sp_addextendedproperty
        @name = N'SchemaSaurus:AggregateRoot',
        @value = N'True',
        @level0type = N'SCHEMA',
        @level0name = N'dbo',
        @level1type = N'TABLE',
...
IF NOT EXISTS (
    ...
    WHERE ep.[class] = 1
      AND ep.[major_id] = OBJECT_ID(N'[dbo].[User]')
      AND ep.[minor_id] = COLUMNPROPERTY(OBJECT_ID(N'[dbo].[User]'), N'EmailAddress', 'ColumnId')
      AND ep.[name] = N'SchemaSaurus:IsSensitive'
)
BEGIN
    EXEC sys.sp_addextendedproperty
        @name = N'SchemaSaurus:IsSensitive',
        @value = N'True',
        @level0type = N'SCHEMA',
        @level0name = N'dbo',
        @level1type = N'TABLE',
...
IF NOT EXISTS (
    ...
    WHERE ep.[class] = 7
      ...
      AND ep.[name] = N'SchemaSaurus:Purpose'
)
BEGIN
    EXEC sys.sp_addextendedproperty
        @name = N'SchemaSaurus:Purpose',
